...

{
	...
	strcpy_s(this->gSensorNames[10], sizeof(this->gSensorNames[10]), "pwr");
	// power supply (get's hot while charging battery)
	...
	if ((tickCount / 1000) <= SecWinUptime)
	{
		...
		if (!NoWaitMessage)
		{
			...
			if (os.dwMajorVersion >= 6 && Runs_as_service == TRUE);
				...
				MessageBox(nullptr, bufsec, "TPFanControl is sleeping", MB_ICONEXCLAMATION);
		}
	}

	// sleep until start time + delay time
	if ((GetTickCount() / 1000) <= static_cast<DWORD>(SecWinUptime))
	{
		while (static_cast<DWORD>(tickCount + SecStartDelay * 1000) >= GetTickCount())
			Sleep(200);
	}
	...
	m_fanTimer = SetTimer(this->hwndDialog, 1, this->Cycle * 1000, nullptr); // fan update
	m_titleTimer = SetTimer(this->hwndDialog, 2, 500, nullptr); // title update
	m_iconTimer = SetTimer(this->hwndDialog, 3, this->IconCycle * 1000, nullptr); // Vista icon update
	if (this->ReIcCycle) m_renewTimer = SetTimer(this->hwndDialog, 4, this->ReIcCycle * 1000, nullptr);
	// Vista icon update
...
FANCONTROL::DlgProc(HWND hwnd, ULONG msg, WPARAM mp1, LPARAM mp2)
{
	...
	case WM_COMMAND:
		if (HIWORD(mp1) == BN_CLICKED || HIWORD(mp1) == EN_CHANGE)
		{
			...
			if (cmd >= 8300 && cmd <= 8302 || cmd == 8310)
			{
				...
				case 5050: // donate
					::ShellExecute(nullptr, "open", Title5,
	...
	case WM__NEWDATA:
		...
		else
		{
			...
			this->Trace(buf);
			sprintf_s(buf, sizeof(buf), "We will close to BIOS-Mode after %d consecutive read errors",
			          this->MaxReadErrors);
			this->Trace(buf);
			this->ReadErrorCount++;
			...
			if (dishow && !this->NoBallons)
			{
				if (Fahrenheit)
				{
					ppTbTextIcon[0]->DiShowballon(
						_T(
							"shows max. temperature in °F and sensor name, left click on icon shows or hides control window, right click shows menue"),
				...
				else
				{
					ppTbTextIcon[0]->DiShowballon(
						_T(
							"shows max. temperature in °C and sensor name, left click on icon shows or hides control window, right click shows menue"),
						_T("TPFanControl new text icon"),NIIF_INFO, 11);
				}
