...
int FANCONTROL::ReadByteFromEC(int offset, char* pdata)
{
	...
	for (iTime = 0; iTime < iTimeoutBuf; iTime += iTick)
	{
		...
		data = static_cast<char>(ReadPort(EC_CTRLPORT)) & 0xff; // or timeout iTimeoutBuf = 1000
		if (!(data & (EC_STAT_IBF | EC_STAT_OBF))) break;
		Sleep(iTick);
	}

	if (data & EC_STAT_OBF) data2 = static_cast<char>(ReadPort(EC_DATAPORT)); //clear OBF if full
	...
	for (iTime = 0; iTime < iTimeout; iTime += iTick)
	{
		...
		data = static_cast<char>(ReadPort(EC_CTRLPORT)) & 0xff;
