...
FANCONTROL::HandleData(void)
{
	...
	for (i = 0; i < 12; i++)
	{
		...
		// name (e.g. "|CPU|") to match against list above

		if (this->State.Sensors[i] != 0x80 && this - State.Sensors[i] != nullptr && strstr(list, what) == nullptr)
		{
	...
	if (this->SlimDialog == 1)
	{
		...
		{
			if (!(SlimDialog && StayOnTop))
				sprintf_s(obuf2 + strlen(obuf2), sizeof(obuf2) - strlen(obuf2), " Non Bios %d", fanctrl & 0x3F);
	...
	if (Fahrenheit)
	{
		for (i = 0; i < 12; i++)
		{
			...
			{
				if (this->State.Sensors[i] != 0) sprintf_s(templist + strlen(templist),
				                                           sizeof(templist) - strlen(templist), "%d;",
...
FANCONTROL::ReadEcRaw(FCSTATE* pfcstate)
{
	...
	neuerversuch :
		...
		{
			// wait for ec ready
			data = static_cast<char>(ReadPort(0x1604)) & 0xff; // or timeout iTimeoutBuf = 1000
		...
		data = static_cast<char>(ReadPort(0x1604)) & 0xff;
		...
		for (int i = 0; i < 16; i++)
		{
			dataOut[i] = static_cast<char>(ReadPort(0x1610 + i)) & 0xff;
		}
