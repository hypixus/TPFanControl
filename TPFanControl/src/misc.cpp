...
FANCONTROL::ReadConfig(const char* configfile)
{
	...
	HANDLE hLockS = CreateMutex(nullptr,FALSE, "TPFanControlMutex01");
	if (hLockS == nullptr)Runs_as_service = TRUE;
	...
	{
	case 5: _SPC = SetPriorityClass(GetCurrentProcess(), REALTIME_PRIORITY_CLASS);
		break;
	case 4: _SPC = SetPriorityClass(GetCurrentProcess(), HIGH_PRIORITY_CLASS);
		break;
	case 3: _SPC = SetPriorityClass(GetCurrentProcess(), ABOVE_NORMAL_PRIORITY_CLASS);
		break;
		// case 2: _SPC = SetPriorityClass(GetCurrentProcess(), NORMAL_PRIORITY_CLASS); break;
	case 1: _SPC = SetPriorityClass(GetCurrentProcess(), BELOW_NORMAL_PRIORITY_CLASS);
		break;
	case 0: _SPC = SetPriorityClass(GetCurrentProcess(), IDLE_PRIORITY_CLASS);
		break;
	...
	sprintf_s(buf, sizeof(buf), "  IgnoreSensors= %s, ProcessPriority= %d, IconCycle= %d", IgnoreSensors,
	          ProcessPriority, IconCycle);
	this->Trace(buf);

	sprintf_s(buf, sizeof(buf), "  BluetoothEDR= %d, NoWaitMessage= %d, ShowBiasedTemps= %d", this->BluetoothEDR,
	          NoWaitMessage, ShowBiasedTemps);
	this->Trace(buf);
	...
	sprintf_s(buf, sizeof(buf), "  ManModeExit= %d, SecWinUptime= %d, SecStartDelay= %d", this->ManModeExit,
	          this->SecWinUptime, this->SecStartDelay);
	this->Trace(buf);
...
FANCONTROL::CreateThread(int (_stdcall *fnct)(ULONG), ULONG p)
{
	...
	DWORD tid;
	HANDLE hThread;
	hThread = ::CreateThread(nullptr, 8 * 4096, thread, (void*)p, 0, &tid);
	return hThread;
}
