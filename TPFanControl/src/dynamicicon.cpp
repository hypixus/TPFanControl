...
CDynamicIcon::CDynamicIcon(const char* line1, const char* line2, const int iFarbeIconA, const int iFontIconA)
{
	...
	oldBmp_1 = static_cast<HBITMAP>(SelectObject(memDC1_, static_cast<HBITMAP>(iconBmp_)));
	oldBmp_2 = static_cast<HBITMAP>(SelectObject(memDC2_, static_cast<HBITMAP>(iconMaskBmp_)));

	// prepare mask
	HBRUSH hBrush = CreateSolidBrush(RGB(255, 255, 255));
	hOldBrush = static_cast<HBRUSH>(SelectObject(memDC2_, hBrush));
	...
	if (hOldFont = static_cast<HFONT>(SelectObject(memDC1_, hfnt)))
	{
		...
		r.bottom = 9; //org 10
		r.right = 15;
		DrawTextEx(memDC1_, static_cast<LPSTR>(_line1), strlen(_line1), &r,DT_CENTER, nullptr);

		r.top = 5; //org 6 neu ?
		r.left = 0;
		r.bottom = 16; // org 15
		r.right = 15;
		DrawTextEx(memDC1_, static_cast<LPSTR>(_line2), strlen(_line2), &r,DT_CENTER, nullptr);
	...
	SelectObject(memDC1_, static_cast<HBITMAP>(oldBmp_1));
	DeleteDC(memDC1_);
	SelectObject(memDC2_, static_cast<HBITMAP>(oldBmp_2));
	DeleteDC(memDC2_);
	DeleteDC(hDC);
