...
CSystemTray* CSystemTray::m_pThis = nullptr;
...
ATOM CSystemTray::RegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpfnWndProc = static_cast<WNDPROC>(WindowProc);
	wcex.cbClsExtra = 0;
	wcex.cbWndExtra = 0;
...
{
	HICON hIcon = static_cast<HICON>(::LoadImage(m_hInstance,
...
BOOL CSystemTray::Animate(UINT nDelayMilliSeconds, int nNumSeconds /*=-1*/)
{
	...
	m_uIDTimer = SetTimer(m_hWnd, m_nTimerID, nDelayMilliSeconds, nullptr);
	return (m_uIDTimer != 0);
}
...
LRESULT CSystemTray::OnTrayNotification(UINT wParam, LONG lParam)
{
...
#endif
	{
		...
		TrackPopupMenu(hSubMenu, 0, pos.x, pos.y, 0, hTargetWnd, nullptr);
...
LRESULT PASCAL CSystemTray::WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (message == m_nTaskbarCreatedMsg)
...
void CSystemTray::MaximiseFromTray(HWND hWnd)
{
	...
	SetWindowLong(hWnd, GWL_STYLE, GetWindowLong(hWnd, GWL_STYLE) | WS_VISIBLE);
	RedrawWindow(hWnd, nullptr, nullptr, RDW_UPDATENOW | RDW_ALLCHILDREN | RDW_FRAME |
