...
{
	this->hmux = ::CreateMutex(nullptr, (state ? TRUE : FALSE), name);
}
...
TASKBARICON::Construct()
{
	...
	if (this->IconId)
	{
		nof.hIcon = static_cast<HICON>(::LoadImage(hInstRes, MAKEINTRESOURCE(this->IconId), IMAGE_ICON, 16, 16,
		                                           LR_DEFAULTCOLOR));
		nof.uFlags |= NIF_ICON;
	}
...
MENU::MENU(int id, HINSTANCE hdll)
	: IsLoaded(TRUE),
	  hMenu(::LoadMenu(((ULONG)hdll == static_cast<ULONG>(-1)) ? hInstRes : hdll, MAKEINTRESOURCE(id))),
	  hWndOwner(nullptr)
{
}
...
MENU::GetSubmenuFromPos(int pos)
{
	...
	rc = static_cast<HMENU>(GetSubMenu(*this, pos));
