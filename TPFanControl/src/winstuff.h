...
public:
	MENU(HWND hwnd);
	MENU(HMENU hm);
	MENU(int id, HINSTANCE hdll = (HINSTANCE)static_cast<ULONG>(-1));
	...
	{
		return static_cast<HMENU>(this->hMenu);
	}
	...
	{
		return this->InsertItem(nullptr, 0, pos);
	}
	...
	int Popup(HWND hwnd, POINT* ppoint = nullptr, BOOL synchtrack = FALSE);
};
