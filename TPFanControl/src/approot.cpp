...
int APIENTRY WinMain(HINSTANCE instance, HINSTANCE, LPSTR aArgs, int)
{
	...
	HANDLE hLock = CreateMutex(nullptr,FALSE, "TPFanControlMutex01");
	...
	if (aArgs && *aArgs)
	{
		...
		if (run)
		{
			...
			svcEntry[1].lpServiceName = nullptr;
			svcEntry[1].lpServiceProc = nullptr;
...
{
	SC_HANDLE SCMgr = OpenSCManager(nullptr, nullptr, SC_MANAGER_ALL_ACCESS);
	if (!SCMgr)
	{
		DWORD ec = GetLastError();
	...
	GetModuleFileName(nullptr, ExePath, sizeof(ExePath));
...
void ShowError(DWORD ec, const char* description)
{
	...
	char* dispBuf = static_cast<char*>(LocalAlloc(LMEM_ZEROINIT, dispBuf_len));
	sprintf_s(dispBuf, dispBuf_len, "%s, error code %d: %s",
	          description, ec, msgBuf);
	MessageBox(nullptr, dispBuf, "Error", MB_OK);
	LocalFree(msgBuf);
	LocalFree(dispBuf);
}
...
{
	MessageBox(nullptr, description, title, MB_OK);
}
...
{
	g_workerThread = (HANDLE)_beginthread(WorkerThread, 0, nullptr);
}
...
void WorkerThread(void* dummy)
{
	...
	hInstRes = GetModuleHandle(nullptr);
	hInstApp = hInstRes;
